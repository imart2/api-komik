...
app.get("/api/komik-populer/:page", async (req, res) => {
  try {
    ...
    // ===============================
    // 4. Scrape DATA HALAMAN INI SAJA
    // ===============================
    $("div#content div.list-update_item").each((i, el) => {
    ...
    res.status(500).json({
      error: "Gagal scraping pagination",
...
// Endpoint Komik Terbaru
app.get("/api/komik-terbaru/:page", async (req, res) => {
  try {
    const page = parseInt(req.params.page) || 1;
    const limit = 12;
...
// POPULAR MANHWAddd
app.get('/api/manhwa-popular-new', async (req, res) => {
  try {
    // URL yang akan di-scrape
    const url = 'https://komiku.org/other/hot/';
    ...
    $('.daftar').each((index, element) => {
      const title = $(element).find('.bge .kan h3').text().trim();
